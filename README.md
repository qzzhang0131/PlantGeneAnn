...
**PlantGeneAnn** (Plant Gene Annotator) is a tool for accurate **ab initio protein-coding gene structure annotation in plant genomes**. It predicts nucleotide-level gene states independently on both genomic strands and converts them into biologically valid gene models through **semi-Markov model (SMM) decoding**.
